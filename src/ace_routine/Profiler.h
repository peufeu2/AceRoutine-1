 ...
 * 
 */
class HistogramCoroutineProfiler : public ace_routine::CoroutineProfilerBase {
protected:
...
protected:
	 ...
	 *  etc
	 */
	virtual void print( Print& printer, unsigned long cycles_per_second ) {
		printer.printf("{\"type\":\"lin\", \"div\":%d, \"hz\": %d, \"runtime_us\": %d, \"data\":", divider, cycles_per_second, micros()-clear_time );
		print_hist( printer );
		printer.print("}");
	}
};
...
protected:
	 ...
	 */
	virtual void print( Print& printer, unsigned long cycles_per_second ) {
		printer.printf("{\"type\":\"log\", \"hz\": %d, \"runtime_us\": %d, \"data\":", cycles_per_second, micros()-clear_time );
		print_hist( printer );
		printer.print("}");
	}
};
